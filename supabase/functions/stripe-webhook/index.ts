...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (event.type === 'customer.subscription.updated' || event.type === 'customer.subscription.created') {
      ...
      const baseDays: Record<string, number> = { 'monthly': 30, 'trimestral': 90, 'semestral': 180, 'anual': 365 }
      const freeMonthsDays: Record<string, number> = { 'monthly': 0, 'trimestral': 0, 'semestral': 30, 'anual': 60 }
      ...
      if (existingSub) {
        ...
        await supabase.from('profiles').update({ is_premium: status === 'active' }).eq('id', existingSub.user_id)
      }
      ...
      if (existingSub) {
        await supabase.from('subscriptions').update({ status: 'cancelled', cancelled_at: new Date().toISOString() }).eq('stripe_customer_id', customerId)
